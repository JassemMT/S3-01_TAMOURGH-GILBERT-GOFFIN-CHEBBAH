...
public class Modele implements Sujet {
    ...
    public void renommerColonne(String ancienNom, String nouveauNom) {
        ...
        if (!colonnesDisponibles.contains(ancienNom)) return;
        if (colonnesDisponibles.contains(nouveauNom)) {
            System.out.println("Ce nom de colonne existe déjà !");
            return;
        }

        // 1. Reconstruire le Set pour garder l'ordre (LinkedHashSet)
        Set<String> nouveauSet = new LinkedHashSet<>();
        for (String col : colonnesDisponibles) {
            if (col.equals(ancienNom)) {
                nouveauSet.add(nouveauNom);
            } else {
                nouveauSet.add(col);
            }
        }
        ...
        // 2. Mettre à jour toutes les tâches qui étaient dans cette colonne
        for (Tache t : taches) {
            if (ancienNom.equals(t.getColonne())) {
                t.setColonne(nouveauNom);
            }
        }
    ...
    public void supprimerColonne(String nomColonne) {
        // On interdit de supprimer "À faire" pour avoir toujours une colonne par défaut
        if ("À faire".equals(nomColonne)) {
            System.out.println("Impossible de supprimer la colonne par défaut.");
            return;
        }

        if (colonnesDisponibles.contains(nomColonne)) {
            // 1. Déplacer les tâches orphelines vers "À faire"
            for (Tache t : taches) {
